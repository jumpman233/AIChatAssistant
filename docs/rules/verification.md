...
* 复杂 calculator success 场景：
  ...
  * `text_delta+`
  * `message_done`
* pending / running / terminal 三次 ToolCall 事件中的 ToolCall id 保持一致
* ToolCall 记录正确落库
* `calculator` success 至少覆盖一个复杂表达式
* `currentTime` success 断言：
  * `iso` 可解析
  * `timeZone = UTC`
  * `formatted` 非空
* ToolCall 事件中的 `assistantMessageId` 必须等于 `toolCall.messageId`
* calculator failed 场景：
  * `tool_call_created(pending)`
  * `tool_call_updated(running)`
  * `tool_call_updated(failed)`
  * assistant 仍 `message_done`
  * 不发送 `message_failed`
* ToolCall failed 不应破坏 assistant message 状态机规则
* 普通非工具消息没有 `tool_call_created` / `tool_call_updated`
