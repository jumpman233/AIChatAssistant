...
启用 `ark` 时，`ARK_API_KEY`、`ARK_BASE_URL`、`ARK_MODEL` 必须存在。`ARK_TIMEOUT_MS` 必须是正整数，语义为上游网络空闲超时，不是生成总时长上限。
...
流式 Provider 的取消、idle timeout、网络活动续期和资源清理规则以 `docs/architecture/provider-streaming-contract.md` 为准。

`ARK_TIMEOUT_MS` 当前语义是 Ark 建连和流式读取期间的空闲超时：连续没有收到任何上游网络数据超过该时间才触发 timeout。它不是整个生成过程的绝对总时长限制。
